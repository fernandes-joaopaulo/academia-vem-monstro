...
if ($_POST['form_operacao'] == "inclusao_aparelhos_aerobicos") {
    try
    {
		...
		$result = $conn->query("SELECT * FROM tb_aparelhos_aerobico where codigo = $codigo");
		$count = $result->rowCount();
		if ($count > 0) {
			$destino = "function () {window.location='inclusao_aparelhos_aerobicos.php';}";
			echo "<script>sendToastr('Código de aparelho já cadastrado!<br />Clique para continuar!','error',$destino)</script>";
		}
		$stmt = $conn->prepare('INSERT INTO tb_aparelhos_aerobico VALUES(:codigo,:nome_aparelho,:marca,:preco,
		:funcao,:velocidade_max,:velocidade_min,:material,:peso,:comentarios)');
...
<br>
<div class="form">
	<h5>INCLUSÃO DE APARELHOS AERÓBICOS</h5>
	<br>
	<form method="POST" action="inclusao_aparelhos_aerobicos.php" name="form_inclusao">
		<table width="600">
		  <tr>
		  ...
		  <tr>
			...
			<td class="td_input">
			  <input type="number" name="velocidade_max" id="velocidade_max" required="required">
			</td>
		  </tr>
		  <tr>
			<td class="td_r">Velocidade Mínima:</td>
			<td class="td_input">
			  <input type="number" name="velocidade_min" id="velocidade_min" required="required">
			</td>
		  </tr>
		  <tr>
		  ...
		  <tr>
			<td colspan='2' class="td_c">
				<br />
				<input type="hidden" name="form_operacao" value="inclusao_aparelhos_aerobicos">
